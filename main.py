...
def download_chromedriver():
    """Download and update ChromeDriver automatically"""
    ...
    sep = '-' * 40
    print(sep)
    print("Link Opener ChromeDriver Downloader")
    print(sep)
...
def ensure_config_exists():
    ...
    if not os.path.exists(config_path):
        # Create initial config with a placeholder URL
        initial_config = {
            "chromedriver_url": "https://storage.googleapis.com/chrome-for-testing-public/130.0.6723.69/win64/chromedriver-win64.zip"
        }
        with open(config_path, 'w', encoding='utf-8') as cf:
            json.dump(initial_config, cf, indent=2)
        print(f"Created initial config.json at {config_path}")


def init_chromedriver():
    """Initialize and update ChromeDriver if needed"""
    try:
        ...
        # Update chromedriver path to use the driver subdirectory
        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
        DRIVER_DIR = os.path.join(BASE_DIR, "driver")
        CHROMEDRIVER_PATH = os.path.join(DRIVER_DIR, "chromedriver.exe")
        ...
        if os.path.exists(CHROMEDRIVER_PATH):
            ...
            # Fallback to legacy path if driver subdirectory doesn't work
            legacy_path = os.path.join(BASE_DIR, "chromedriver.exe")
            if os.path.exists(legacy_path):
                print(f"Using legacy ChromeDriver at: {legacy_path}")
                return legacy_path
            else:
                raise FileNotFoundError("ChromeDriver not found after update attempt")
                
    except Exception as e:
        print(f"Warning: ChromeDriver update failed: {e}")
        # Try to use existing chromedriver.exe in base directory as fallback
        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
        legacy_path = os.path.join(BASE_DIR, "chromedriver.exe")
        if os.path.exists(legacy_path):
            print(f"Using existing ChromeDriver at: {legacy_path}")
            return legacy_path
        else:
            raise Exception(f"ChromeDriver not available and update failed: {e}")
...
class LinkOpenerWorker(QThread):
    ...
    def setup_chrome_driver(self):
        """Setup Chrome driver dengan incognito mode"""
        try:
            # Use the updated chromedriver path from init
            chromedriver_path = init_chromedriver()
...
class LinkOpenerApp(QMainWindow):    
    ...
    def setup_chrome_driver_for_single_link(self):
        ...
        try:
            # Use the initialized chromedriver path
